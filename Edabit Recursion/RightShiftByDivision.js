...
//  const num = y => y === 0 ? 1 : 2 * num(y - 1);
//  return x/num(y)
// }

// const shiftToRight = (x, y) => (y ? shiftToRight(Math.floor(x / 2), --y) : x);

const shiftToRight2 = (x, y) => (y ? shiftToRight(x / 2, y - 1) : Math.floor(x));

const shiftToRight = (x, y) => {
  if(y) return shiftToRight(x/2, y-1) 
  return Math.floor(x)
}
